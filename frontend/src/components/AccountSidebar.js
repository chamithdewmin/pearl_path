...
const links = [
  { to: '/account', end: true, label: 'Overview', Icon: null },
  { to: '/account/hotels', label: 'Hotels', Icon: HotelIcon },
  { to: '/account/vehicles', label: 'Vehicles', Icon: CarIcon },
  { to: '/account/guides', label: 'Guides', Icon: GuideIcon },
  { to: '/account/bookings', label: 'My bookings', Icon: BookingIcon },
  { to: '/account/profile', label: 'Profile', Icon: ProfileIcon },
];
...
const s = {
  ...
  adminLink: { display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--color-primary)', textDecoration: 'none' },
...
export default function AccountSidebar({ collapsed, onToggleCollapse, onHide }) {
  const { user } = useAuth();
  ...
  return (
    <aside style={sidebarStyle(collapsed)}>
      <div style={s.header}>
        <span style={s.logo}>{collapsed ? 'ST' : 'My account'}</span>
