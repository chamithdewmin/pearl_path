...
export default function AccountHotels() {
  ...
  return (
    <div style={s.wrapper}>
      <div style={s.header}>
        ...
        <p style={s.subtitle}>Browse and book hotels across Southern Sri Lanka.</p>
      </div>
      <div style={s.toolbar}>
        <div style={s.searchBar}>
          <SearchIcon size={22} color="var(--color-text-muted)" />
          <input type="text" placeholder="Search hotels..." className="input-enterprise" style={s.searchInput} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
        </div>
      </div>
