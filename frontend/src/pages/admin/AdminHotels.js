...
const MOCK = [
  { id: 1, name: 'Pavana Hotel - Galle', location: 'Galle', price: 'LKR 12,000' },
  { id: 2, name: 'Jetwing Lighthouse', location: 'Galle Fort', price: 'LKR 55,000' },
];
  ...
  return (
    <div>
      ...
      <div className="card-enterprise" style={s.tableWrap}>
        <table style={s.table}>
          <thead><tr><th style={s.th}>Name</th><th style={s.th}>Location</th><th style={s.th}>Price</th><th style={s.th}>Actions</th></tr></thead>
          <tbody>
            {MOCK.map((h) => (
              <tr key={h.id} style={s.tr}>
                <td style={s.td}>{h.name}</td>
                <td style={s.td}>{h.location}</td>
                <td style={s.td}>{h.price}</td>
